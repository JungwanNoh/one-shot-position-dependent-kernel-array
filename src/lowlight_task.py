...
def run():
    ...
    for idx, name in enumerate(names):
        ...
        rows.append({'name': name, 'psnr_raw': m_raw['psnr'], 'psnr_global': m_global['psnr'], 'psnr_pdk': m_pdk['psnr'], 'ssim_raw': m_raw['ssim'], 'ssim_global': m_global['ssim'], 'ssim_pdk': m_pdk['ssim'], 'grad_raw': m_raw['grad'], 'grad_global': m_global['grad'], 'grad_pdk': m_pdk['grad']})
        ...
        if idx < 8:
            save_panel(os.path.join(panel_dir, f'{name}_panel.png'), global_out, pdk_out, zone, gt=gt)
        print(
            f"[{idx+1:02d}] {name} | "
            f"mean input/gt/global/pdk = {observed.mean():.4f} / {gt.mean():.4f} / {global_out.mean():.4f} / {pdk_out.mean():.4f} | "
            f"PSNR raw/global/pdk = {m_raw['psnr']:.3f} / {m_global['psnr']:.3f} / {m_pdk['psnr']:.3f}"
        )
