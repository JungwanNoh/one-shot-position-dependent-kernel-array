...
def run():
    ...
    for idx, path in enumerate(files):
        ...
        rows.append({'name': name, 'psnr_observed': m_obs['psnr'], 'psnr_global': m_global['psnr'], 'psnr_pdk': m_pdk['psnr'], 'ssim_observed': m_obs['ssim'], 'ssim_global': m_global['ssim'], 'ssim_pdk': m_pdk['ssim'], 'grad_observed': m_obs['grad'], 'grad_global': m_global['grad'], 'grad_pdk': m_pdk['grad']})
        ...
        if idx < 8:
            save_panel(os.path.join(panel_dir, f'{name}_panel.png'), global_out, pdk_out, zone, gt=clean)
