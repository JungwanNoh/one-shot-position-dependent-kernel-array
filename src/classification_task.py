...
def train_single_mode(clf, train_loader, test_loader, device, mode: str, kernel_bank=None):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        scheduler.step()
        acc, _ = evaluate_classifier(clf, test_loader, device, mode, kernel_bank)
        if acc > best_acc:
            best_acc = acc
            best_state = copy.deepcopy(clf.state_dict())
        logs.append({'epoch': epoch, 'ce': float(sum(ce_vals)/len(ce_vals)), 'test_acc': acc, 'best_acc': best_acc})
        print(f"[{mode.upper()}] Epoch {epoch:03d} | CE={sum(ce_vals)/len(ce_vals):.5f} | Test Acc={acc:.4f} | Best={best_acc:.4f}")
...
def run():
    ...
    with open(os.path.join(SAVE_ROOT, f'{DATASET_NAME.lower()}_raw_log.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=list(raw_logs[0].keys())); writer.writeheader(); writer.writerows(raw_logs)
    with open(os.path.join(SAVE_ROOT, f'{DATASET_NAME.lower()}_global_log.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=list(global_logs[0].keys())); writer.writeheader(); writer.writerows(global_logs)
    with open(os.path.join(SAVE_ROOT, f'{DATASET_NAME.lower()}_pdk_log.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=list(pdk_logs[0].keys())); writer.writeheader(); writer.writerows(pdk_logs)
    summary_rows = [
