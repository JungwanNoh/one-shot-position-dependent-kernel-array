...
def save_panel(save_path: str, clean: np.ndarray, observed: np.ndarray, global_out: np.ndarray,
               pdk_out: np.ndarray, gaze_map: np.ndarray, base_zone: np.ndarray, final_zone: np.ndarray):
    ...
    axes[0, 2].imshow(gaze_map, cmap="magma", vmin=0, vmax=1)
    axes[0, 2].set_title("Gaze Map")
    axes[0, 2].axis("off")

    axes[0, 3].imshow(final_zone, cmap="viridis", vmin=0, vmax=2)
    axes[0, 3].set_title("Final Zone Map")
    axes[0, 3].axis("off")
    ...
    axes[1, 1].set_title("Gaze/Sensor PDK")
    axes[1, 1].axis("off")
